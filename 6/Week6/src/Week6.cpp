...
bool x_orbit_toggled_on = false;
bool y_orbit_toggled_on = false;
bool wireframe_toggled_on = false;
bool export_images_toggled_on = false;
int ppm_count = 0;
	...
	if (objFile.is_open()) {
		...
		while (getline(objFile, line)){
			...
			if (token == "mtllib") {
				...
				TexturePoint currentTexturePoint = TexturePoint(currentPoint[0], currentPoint[1]);
				texturePoints.push_back(currentTexturePoint);
				//std::cout<<"("<<currentTexturePoint.x<<", "<<currentTexturePoint.y<<")\n";
			} else if (token == "f") {
				if (containsTextures == true) {
					std::cout<<"fixed f?\n";
					delimiter = "/";
					int vertexIndex;
				...
				else if (containsTextures == false){
					delimiter = "/";
					int vertexIndex;
					ModelTriangle currentTriangle = ModelTriangle();
					for (int i = 0; i < 3; i++) {
...
void lookAt(glm::vec3 point){
	glm::vec3 forward_direction = glm::normalize(glm::vec3(cameraPosition[0]-point[0],cameraPosition[1]-point[1],cameraPosition[2]-point[2]));
	glm::vec3 right_direction = glm::normalize(glm::cross(glm::vec3(0.0, 1.0, 0.0),forward_direction));
	glm::vec3 up_direction = glm::cross(forward_direction, right_direction);

	cameraOrientation[0] = right_direction;
	cameraOrientation[1] = up_direction;
	cameraOrientation[2] = forward_direction;
}

void x_orbit(){
	float theta = PI /180;
	if (x_orbit_toggled_on) {
		cameraPosition = xRotation(theta) * cameraPosition;
		//cameraOrientation = yRotation(theta) * cameraOrientation;
...
void handleEvent(SDL_Event event, DrawingWindow &window) {
	if (event.type == SDL_KEYDOWN) {
		...
		if (event.key.keysym.sym == SDLK_LEFT) {
			...
			std::cout << "TOGGLED X ORBIT" << std::endl;
			x_orbit_toggled_on = !x_orbit_toggled_on;
		}else if (event.key.keysym.sym == SDLK_o) {
			std::cout << "TOGGLED Y ORBIT" << std::endl;
			y_orbit_toggled_on = !y_orbit_toggled_on;
		}else if (event.key.keysym.sym == SDLK_h) {
			std::cout << "TOGGLED WIREFRAME" << std::endl;
			wireframe_toggled_on = !wireframe_toggled_on;
		}else if (event.key.keysym.sym == SDLK_r) {
			std::cout << "TOGGLED EXPORT OF OUTPUT IMAGES" << std::endl;
			export_images_toggled_on = !export_images_toggled_on;
		}
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if (export_images_toggled_on){
			std::string frame_name = std::to_string(ppm_count) + ".ppm";
			window.savePPM(frame_name);
			ppm_count++;
		}
